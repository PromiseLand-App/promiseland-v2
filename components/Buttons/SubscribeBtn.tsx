...
import { pollRelayActionStatus } from "@/lib/functions";
...
function SubscribeBtn({
    ...
    try {
      ...
      const subscribe = async (id: string) => {
        ...
        if (res.txHash) {
          ...
          handleModal(
            "success",
            `Your successfully subscribed https://testnet.bscscan.com/tx/${res.txHash}}`,
          );
          return;
        }
  ...
  return (
    <button
      className={`flex items-center rounded-full border border-black px-3 py-1 text-xs duration-100 ease-in hover:bg-stone-900 hover:text-white ${
