...
import { PROFILE_NFT_OPERATOR } from "@/lib/constants";
import { pinJSONToIPFS } from "@/lib/functions";
import {
  randUserName,
  randAvatar,
  randPhrase,
...
function SignupBtn({ handle, avatar, name, bio, operator }: ISignupInput) {
  ...
  const [
    createTypedData,
    {
      data: dataCreateTypedData,
      loading: loadingCreateTypedData,
      error: errorCreateTypedData,
    },
  ] = useMutation(CREATE_CREATE_PROFILE_TYPED_DATA);
  const [relay, { data: relayData, loading: relayLoading, error: relayError }] =
    useMutation(RELAY);
  const [
    relayActionStatus,
    {
  ...
  const handleOnClick = async () => {
    try {
      ...
      const createTypedDataResult = await createTypedData({
        variables: {
          to: address,
      ...
      const typedDataId =
        createTypedDataResult.data?.createCreateProfileTypedData?.typedDataID;
      console.log("typedDataId", typedDataId);

      const relayResult = await relay({
        variables: { input: { typedDataID: typedDataId } },
      });
      ...
      const pollRelayActionStatus = async (id: string) => {
        ...
        const relayActionStatusResult = await relayActionStatus({
        ...
        if (relayActionStatusResult.data?.relayActionStatus?.txHash) {
          ...
          alert(
            "Error: " + relayActionStatusResult.data?.relayActionStatus?.reason,
          );
          return;
        }
      ...
      const profileIdData = await profileByHandle({
        variables: { handle: handle || randUserName() },
      });
